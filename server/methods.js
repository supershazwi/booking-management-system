...
var qbo = new QuickBooks('qyprdv7Uh8xyBXjescVfQWgZmGWOls',
        ...
        address.Line1 = customerAttributes['address'];

        qbo.createCustomer({
          BillAddr: address,
          CompanyName: customerAttributes['company'], 
          DisplayName: customerAttributes['name'],
          FamilyName: customerAttributes['lastName'],
          GivenName: customerAttributes['firstName'],
          MiddleName: customerAttributes['middleName'],
          Active: true,
          PrimaryPhone: {
            FreeFormNumber: customerAttributes['contact']
          },
          PrimaryEmailAddr: {
            Address: customerAttributes['email']
          }
          ...
          if(customer.Fault != undefined) {
          }
          if(customer.Fault != undefined) {
            if(customer.Fault.Error[0].Message != undefined) { 
              CustomerUpdates.update({customerId: "0"}, {$set: {status: customer.Fault.Error[0].Message}});
            } 
          }
          if(customer.Id != undefined) {
            ...
            customerObject.name = customerAttributes['name'];
            ...
            customerObject.quickbooksId = customer.Id;
            customerObject.logs = [];
            var log = new Object();
            ...
            customerUpdate.route = customerId;
...
var qbo = new QuickBooks('qyprdv7Uh8xyBXjescVfQWgZmGWOls',
        ...
        addressLine.Line1 = customerAttributes.address;


        qbo.getCustomer(customerAttributes.customer.quickbooksId, Meteor.bindEnvironment(function(err, customer) {
          if(customer.fault != undefined && customer.fault.type == "AUTHENTICATION") {
            return;
          }

          qbo.updateCustomer({
            Id: customerAttributes.customer.quickbooksId,
            ...
            if (err) {
              console.log("ERROR!!!!!");
              CustomerUpdates.update({customerId: customerAttributes.customer._id}, {$set: {status: err}});
            }
            else {
              if(customer.Fault == undefined) {
                ...
                CustomerUpdates.update({customerId: customerAttributes.customer._id}, {$set: {status: customer.Fault.Error[0].Message}});
              }
            }
          }));
        }));
        ...
        if(customer2.quickbooksId == null || customer2.quickbooksId == 0 || customer2.quickbooksId == "0" || customer2.quickbooksId == "") {
              ...
              if(customer.fault != undefined && customer.fault.type == "AUTHENTICATION") {
                ...
                if(other.equipmentDetails[0].items.length > 0) {
                  for(x in other.equipmentDetails) {
                    ...
                    for(y in other.equipmentDetails[x].items) {
                        var lineObject = new Object();
                        if(other.status == "Void") {
                          lineObject.Amount = 0;
                        } else {
                          lineObject.Amount = other.equipmentDetails[x].items[y].price * other.equipmentDetails[x].items[y].quantity;
                        } 
                        
                        lineObject.DetailType = "SalesItemLineDetail";
                        lineObject.Description = other.equipmentDetails[x].items[y].item;
                        lineObject.SalesItemLineDetail = new Object();
                        lineObject.SalesItemLineDetail.ItemRef = new Object();

                        var coa = ChartOfAccounts.findOne({category: other.equipmentDetails[x].items[y].category});
                        ...
                        if(other.status == "Void") {
                          ...
                          lineObject.SalesItemLineDetail.UnitPrice = other.equipmentDetails[x].items[y].price;
                        } 
                        
                        lineObject.SalesItemLineDetail.Qty = other.equipmentDetails[x].items[y].quantity;
                        lineArray.push(lineObject);

                        //check discount
                        if(other.equipmentDetails[x].items[y].discount > 0) {
                          var lineObject = new Object();
                          if(other.status == "Void") {
                            lineObject.Amount = 0;
                          } else {
                            lineObject.Amount = -(other.equipmentDetails[x].items[y].discount);
                          } 
                          lineObject.DetailType = "SalesItemLineDetail";
                          lineObject.Description = other.equipmentDetails[x].items[y].item + " - DISCOUNT";
                          lineObject.SalesItemLineDetail = new Object();
                          lineObject.SalesItemLineDetail.ItemRef = new Object();
                          ...
                          if(other.status == "Void") {
                            ...
                            lineObject.SalesItemLineDetail.UnitPrice = -other.equipmentDetails[x].items[y].discount;
                          }
                          lineObject.SalesItemLineDetail.Qty = 1;
                          lineArray.push(lineObject);
                        }
                ...
                if(other.quickbooksInvoiceQueryId == "Pending") {
                  ...
                  qbo.createInvoice({
                    ...
                    if(invoice.Fault != undefined) {
                      ...
                      other.quickbooksInvoiceId = invoice.DocNumber;
                      other.quickbooksInvoiceQueryId = invoice.Id;
                    ...
                    qbo.getInvoice(other.quickbooksInvoiceQueryId, Meteor.bindEnvironment(function (error, invoice) {
                      if(invoice.fault != undefined && invoice.fault.type == "AUTHENTICATION") {
                        return;
                      }
                      ...
                      if(invoice.Fault != undefined) {
                        ...
                        BookingUpdates.update({invoiceId: bookingId}, {$set: bookingUpdate});
                            ...
                            InvoiceNeedingUpdate.remove({otherId: other._id});
            ...
            if(other.quickbooksInvoiceQueryId == "Pending") {
              ...
              qbo.createInvoice({
                ...
                if(invoice != undefined && invoice.Fault != undefined) {
                  var bookingUpdate = BookingUpdates.findOne({invoiceId: bookingId});
                  bookingUpdate.status = invoice.Fault.Error[0].Message;
